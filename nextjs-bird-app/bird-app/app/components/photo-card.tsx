...
export default function PhotoCard({...params}){
    return (
        <div className="max-w-sm bg-white border border-gray-200 shadow dark:bg-gray-800 dark:border-gray-700">
            ...
            <div className="p-5">
                <a href={`/photos/${params.slug}`}>
                    <h5 className="mb-2 text-2xl font-bold tracking-tight text-slate-900 dark:text-white">{params.title}</h5>
                </a>
                <a href={`/photos/${params.slug}`} className="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-slate-900 hover:text-slate-900 hover:bg-violet-100 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                    View photo
                </a>
            </div>
        </div>
